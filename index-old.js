...
async function generateProjectReport(data) {
  const compileFunction = pug.compileFile("./reports/templates/project.pug");
  await fs.writeFile(
...
async function listPunchByProject({ project, s, short, h, html, ...rest }) {
  try {
    ...
    if (h || html) {
      return await generateProjectReport(
        punchList.map((punch) => ({
          id: punch.id,
          ...
          duration: JSON.stringify(
            humanize(punch.punchOutAt - punch.punchInAt)
          ),
          note: punch.note,
        }))
      );
